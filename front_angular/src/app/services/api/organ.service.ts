...
export class OrganService {
  ...
  createOrgan(projectUuid: string, req: CreateOrganRequest): Observable<CreateOrganResponse> {
    return this.http.post<CreateOrganResponse>(`/api/projects/${projectUuid}/organs`, req).pipe(
  ...
  updateOrgan(projectUuid: string, organUuid: string, req: UpdateOrganRequest): Observable<UpdateOrganResponse> {
    return this.http.put<UpdateOrganResponse>(`/api/projects/${projectUuid}/organs/${organUuid}`, req).pipe(
      tap(() => this.projectService.clearCache(projectUuid))
    );
  }

  restoreOrgan(projectUuid: string, organUuid: string): Observable<RestoreOrganResponse> {
    return this.http.post<RestoreOrganResponse>(`/api/projects/${projectUuid}/organs/${organUuid}/restore`, {}).pipe(
      tap(() => this.projectService.clearCache(projectUuid))
    );
  }
